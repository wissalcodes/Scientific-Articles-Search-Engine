...
export const ProfileCard = ({ profile, role }) => {
  ...
  //track when the fields are being edited
  ...
  // function to POST passwords to the resetting password API endpoint for moderator
  ...
  // function to POST passwords to the resetting password API endpoint for user
  ...
  // function to post edited user or moderator information to the API
  ...
  // re-render component when the profile changes values
  useEffect(() => {
    setEditedValues({
      first_name: profile.first_name,
      last_name: profile.last_name,
  ...
  return (
    <div
      className={`bg-[#F5EAAB]  drop-shadow py-[10px] font-lora rounded-br-[14px] top-1/3 xl:top-1/4 fixed left-0 -translate-y-[20vh] w-[350px] lg:w-[370px] 
        h-[400px]
       transform transition-transform duration-500 ease-in-out ${
         showCard
           ? "translate-x-0"
           : "-translate-x-[350px] lg:-translate-x-[370px]"
       }`}>
      <div className="px-[20px] flex flex-col  items-start h-full w-full">
        <h1 className="z-30 text-xl lg:text-[32px] pt-[10px] font-bold font-merryweather">
          Mon Profile
        </h1>
        ...
        <div className="grid grid-cols-[30%,50%,20%] w-full items-center">
          ...
          {editing ? (
            ...
            // Only render the "Edit" image if the username is not "admin" (amin can't change his own credentials)
        ...
        <div className={`lg:pl-[40px] grid grid-cols-[60%,40%] w-full `}>
        ...
        <div className={`lg:pl-[40px] grid grid-cols-[60%,40%] w-full `}>
          ...
          {/* call corresponding resetting function */}
          <button
            onClick={
              profile.username === "admin"
                ? resetPasswordAdmin
